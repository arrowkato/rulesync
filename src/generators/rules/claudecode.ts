...
async function updateClaudeSettings(settingsPath: string, ignorePatterns: string[]): Promise<void> {
  ...
  // Use raw settings to preserve all existing properties
  ...
  if (!Array.isArray(permissionsObj.deny)) {
    permissionsObj.deny = [];
  }

  // Remove existing Read() rules that were generated by rulesync
  const denyArray = permissionsObj.deny;
  ...
  // Remove duplicates
  permissionsObj.deny = Array.from(new Set(filteredDeny));
